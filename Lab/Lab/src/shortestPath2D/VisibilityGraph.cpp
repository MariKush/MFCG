	...
	std::vector<Point> VisibilityGraph::visibleVertices(const Point &pivot, const std::vector<Polygon> &obstacles, PointRef pivotRef) {
		...
		for (auto it = obstacles.begin(); it != obstacles.end(); ++it) {
			...
			for (auto pointIt = polygon.begin(); pointIt != polygon.end(); ++pointIt) {
				...
				points.push_back(std::make_pair(PointRef({ static_cast<size_t>(std::distance(obstacles.begin(), it)), static_cast<size_t>(std::distance(polygon.begin(), pointIt)), globalPointCounter++ }), polarPoint));
			}
		}
		...
		for (auto pointIt = points.begin(); pointIt != points.end(); ++pointIt) {
			...
			size_t ccwLineSegment = pointIt->first.localPointId > 0 ? pointIt->first.globalPointId - 1 : pointIt->first.globalPointId + obstacles[pointIt->first.polygonId].size() - 1;
